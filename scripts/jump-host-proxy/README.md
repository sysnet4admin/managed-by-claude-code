...
1. **SSH Tunnel Creation**
   - Establishes SSH connection from local machine to Bastion Host
   - Forwards local port (8443) to remote resource (Private Kubernetes API Server)

2. **kubeconfig Transformation** (for Kubernetes)
   - Converts Private IP in original kubeconfig to localhost:8443
   - Adds TLS verification bypass (insecure-skip-tls-verify)

3. **Local Access**
   - kubectl commands connect to localhost:8443
   - SSH tunnel automatically routes traffic through Bastion Host to Private resources

4. **Transparent Proxy**
   - Users access resources as if they were local
   - All traffic securely routed through SSH tunnel
